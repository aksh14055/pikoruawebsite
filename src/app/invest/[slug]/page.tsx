...
export function generateStaticParams() {
  return getContentHubsByPrefix("invest").map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const page = getContentHubPage(slug);
  if (!page || page.prefix !== "invest") return {};
...
export default async function InvestPage({ params }: PageProps) {
  const { slug } = await params;
  const page = getContentHubPage(slug);
  if (!page || page.prefix !== "invest") notFound();
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Investment Guides", item: absoluteUrl("/invest") },
