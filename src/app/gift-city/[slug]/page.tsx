...
export function generateStaticParams() {
  return getContentHubsByPrefix("gift-city").map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const page = getContentHubPage(slug);
  if (!page || page.prefix !== "gift-city") return {};
...
export default async function GiftCityPage({ params }: PageProps) {
  const { slug } = await params;
  const page = getContentHubPage(slug);
  if (!page || page.prefix !== "gift-city") notFound();
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "GIFT City", item: absoluteUrl("/gift-city") },
